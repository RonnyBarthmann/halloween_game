...
function unified_inventory.get_formspec(player, page)
	...
	if not draw_lite_mode then
		formspec[n] =
			...
			.. "tooltip[start_list;" .. F(S("Erste Seite")) .. "]"
	...
	if not draw_lite_mode then
		...
		formspec[n+1] = "image_button[12.2,8.1;.8,.8;ui_search_icon.png;searchbutton;]"
			.. "tooltip[searchbutton;" ..F(S("Suchen")) .. "]"
		formspec[n+2] = "image_button[12.9,8.1;.8,.8;ui_reset_icon.png;searchresetbutton;]"
			.. "tooltip[searchbutton;" ..F(S("Suchen")) .. "]"
			.. "tooltip[searchresetbutton;" ..F(S("Suchfeld leeren")) .. "]"
	else
	...
	local no_matches = S("Keine passenden Elemente vorhanden")
	if draw_lite_mode then
		no_matches = S("Keine Treffer.")
	end
	...
	if #unified_inventory.filtered_items_list[player_name] == 0 then
		...
		formspec[n] = "label[8.2,"..ui_peruser.form_header_y..";"..F(S("Seite")) .. ": "
			.. S("%s/%s"):format(page,pagemax).."]"
	end
	n= n+1
