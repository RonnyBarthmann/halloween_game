...
unified_inventory.register_button("home_gui_set", {
	...
	action = function(player)
		...
		if minetest.check_player_privs(player_name, {home=true}) then
			...
			minetest.chat_send_player(player_name,
				S("Du hast keine erlaubnis \"home\" zu benutzen!"))
...
unified_inventory.register_button("misc_set_day", {
	...
	action = function(player)
		...
		if minetest.check_player_privs(player_name, {settime=true}) then
			...
			minetest.chat_send_player(player_name,
				S("Stellt die Uhr auf 6 Uhr morgens."))
		else
			minetest.chat_send_player(player_name,
				S("Du hast keine erlaubnis /settime zu benutzen!"))
			unified_inventory.set_inventory_formspec(player, unified_inventory.current_page[player_name])
		end
	end,
	condition = function(player)
...
unified_inventory.register_button("misc_set_night", {
	...
	action = function(player)
		...
		if minetest.check_player_privs(player_name, {settime=true}) then
			...
			minetest.chat_send_player(player_name,
					S("Stellt die Uhr auf 6 Uhr abends."))
		else
			minetest.chat_send_player(player_name,
					S("Du hast keine erlaubnis /settime zu benutzen!"))
			unified_inventory.set_inventory_formspec(player, unified_inventory.current_page[player_name])
...
unified_inventory.register_button("clear_inv", {
	...
	tooltip = S("Inventar Entfernen"),
	action = function(player)
		local player_name = player:get_player_name()
		if not unified_inventory.is_creative(player_name) then
			minetest.chat_send_player(player_name,
					S("Alle Items werden entfernt, abgesehen"
					.." von denen die sich im Craftgrid oder"
					.." in den Belohnungen befinden."
					.."\nVerwende den Papierkorb um einzelne Items zu entfernen."))

			unified_inventory.set_inventory_formspec(player, unified_inventory.current_page[player_name])
			return
		end
		player:get_inventory():set_list("main", {})
		minetest.chat_send_player(player_name, S('Inventar Entfernt!'))
...
unified_inventory.register_page("craft", {
	get_formspec = function(player, perplayer_formspec)
		...
		formspec = formspec.."label[0,"..formheadery..";" ..F(S("Craften")).."]"
		...
		if unified_inventory.trash_enabled or unified_inventory.is_creative(player_name) or minetest.get_player_privs(player_name).give then
			formspec = formspec.."label[7,"..(formspecy + 1.5)..";" .. F(S("Abfall:")) .. "]"
		...
		if unified_inventory.is_creative(player_name) then
			formspec = formspec.."label[0,"..(formspecy + 1.5)..";" .. F(S("Aufstocken:")) .. "]"
...
local function stack_image_button(x, y, w, h, buttonname_prefix, item)
	...
	if show_is_group then
		...
		local grouptip
		if andcount == 1 then
			grouptip = string.format(S("Jedes Item, das zur %s-Gruppe gehoert"), groupstring)
		elseif andcount > 1 then
			grouptip = string.format(S("Alle Items, die zu dieser Gruppe gehoeren: %s"), groupstring)
		end
		grouptip = F(grouptip)
		if andcount >= 1 then
...
local recipe_text = {
	recipe = S("Rezept %d/%d"),
	usage = S("Verwendung %d/%d"),
}
local no_recipe_text = {
	recipe = S("Kein Rezept"),
	usage = S("Keine Verwendung"),
}
local role_text = {
	recipe = S("Ergebnis"),
	usage = S("Zutaten"),
...
unified_inventory.register_page("craftguide", {
	get_formspec = function(player, perplayer_formspec)
		...
		local formspec = ""
		formspec = formspec.."background[0,"..(formspecy + 3.5)..";8,4;ui_main_inventory.png]"
		formspec = formspec.."label[0,"..formheadery..";" .. F(S("Rezepte")) .. "]"
		formspec = formspec.."listcolors[#00000000;#00000000]"
		...
		if not craft then
			...
			if has_creative then
				formspec = formspec.."label[0,"..(formspecy + 2.10)..";" .. F(S("Gib mir:")) .. "]"
						.."button[0,  "..(formspecy + 2.7)..";0.6,0.5;craftguide_giveme_1;1]"
						.."button[0.6,"..(formspecy + 2.7)..";0.7,0.5;craftguide_giveme_10;10]"
						.."button[1.3,"..(formspecy + 2.7)..";0.8,0.5;craftguide_giveme_64;64]"
			end
			return {formspec = formspec}
		end
		...
		for x = 1, display_size.width do
			...
			formspec = formspec.."label["
				...
				..";"..F(S("Dieses Rezept ist zu\ngroß, um es anzuzeigen.")).."]"
		end

		if craft_type.uses_crafting_grid and display_size.width <= 3 then
			formspec = formspec.."label[0,"..(formspecy + 0.9)..";" .. F(S("Rezept:")) .. "]"
					.."button[0,  "..(formspecy + 1.5)..";0.6,0.5;craftguide_craft_1;1]"
					.."button[0.6,"..(formspecy + 1.5)..";0.7,0.5;craftguide_craft_10;10]"
					.."button[1.3,"..(formspecy + 1.5)..";0.8,0.5;craftguide_craft_max;" .. F(S("Max")) .. "]"
		end
		if has_creative then
			formspec = formspec.."label[0,"..(formspecy + 2.1)..";" .. F(S("Gib mir:")) .. "]"
					.."button[0,  "..(formspecy + 2.7)..";0.6,0.5;craftguide_giveme_1;1]"
					.."button[0.6,"..(formspecy + 2.7)..";0.7,0.5;craftguide_giveme_10;10]"
					.."button[1.3,"..(formspecy + 2.7)..";0.8,0.5;craftguide_giveme_64;64]"
		end
...
local function craftguide_craft(player, formname, fields)
	...
	--[[
	if amount == "max" then
		amount = 64 -- Arbitrary; need better way to do this.
	else
		amount = tonumber(amount)
	end]]--
