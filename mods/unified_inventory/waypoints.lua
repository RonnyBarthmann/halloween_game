...
local hud_colors = {
	{"#FFFFFF", 0xFFFFFF, S("Weiss")},
	{"#DBBB00", 0xf1d32c, S("Gelb")},
	{"#DD0000", 0xDD0000, S("Rot")},
	{"#2cf136", 0x2cf136, S("Gruen")},
	{"#2c4df1", 0x2c4df1, S("Blau")},
}
	...
	get_formspec = function(player)
		...
		formspec = formspec ..
			...
			"tooltip[set_waypoint" .. i .. ";"
				.. F(S("Wegpunkt auf aktuelle Position setzen")).."]"
		...
		formspec = formspec ..
			...
			"tooltip[toggle_waypoint" .. i .. ";"
				.. F(S("Wegpunkt setzen @1",
					waypoint.active and S("Unsichtbar") or S("Sichtbar"))).."]"
		...
		formspec = formspec ..
			...
			"tooltip[toggle_display_pos" .. i .. ";"
				.. F(S("@1 anzeigen der Wegpunktkoordinaten",
					waypoint.display_pos and S("Aus") or S("Ein"))) .."]"
		...
		formspec = formspec ..
			...
			"tooltip[toggle_color" .. i .. ";"
				.. F(S("Farbe der Wegpunktanzeige bearbeiten")).."]"
		...
		formspec = formspec ..
			...
			"tooltip[rename_waypoint" .. i .. ";"
				.. F(S("Wegpunktname bearbeiten")).."]"

		-- Waypoint's info:
		if waypoint.active then
			formspec = formspec .. 	"label[1,0.8;"..F(S("Wegpunkte Ein")).."]"
		else
			formspec = formspec .. 	"label[1,0.8;"..F(S("Wegpunkte Aus")).."]"
		end
...
local function update_hud(player, waypoints, temp, i)
	...
	if waypoint.display_pos then
		...
		name = waypoint.name or "Wegpunkt "..i
	end
	if temp.hud then
