...
minetest.after(0.01, function()
	...
	for _, name in ipairs(unified_inventory.items_list) do
		...
		elseif type(def.drop) == "table" then
			...
			the total number of guaranteed drops and drops by chance (â€œmaybesâ€) for each item.
			For â€œmaybesâ€, the final count is the theoretical maximum number of items, not
			...
			-- for the first â€œmaybeâ€ item it will become false which will cause ALL
			-- later items to be considered â€œmaybesâ€.
			-- A common idiom is:
			-- { max_items 1, { items = {
			--	{ items={"example:1"}, rarity = 5 },
			-- 	{ items={"example:2"}, rarity = 1 }, }}}
			-- example:2 must be considered a â€œmaybeâ€ because max_items is set and it
			-- appears after a â€œmaybeâ€
			...
			for i=1,#drop_items do
				...
				for j=1,#itit.items do
					...
					if not dstack:is_empty() and dstack:get_name() ~= name then
						...
						-- Guaranteed drops AND we are not yet in â€œmaybe modeâ€
...
unified_inventory.register_craft_type("digging_chance", {
	description = "Abbauen (mit Chance)",
