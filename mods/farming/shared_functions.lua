...
function farming:add_plant(identifier, full_grown, names, interval, chance)
	...
	minetest.register_abm({
		...
		action = function(pos, node)
			if minetest.get_node({x=pos.x, y=pos.y-1, z=pos.z}).name ~= "farming:soil_wet" and math.random(0, 9) > 0 then
				return
			else
				farming:grow_plant(identifier, pos, node)
			end
		end,
	})
end

function farming:grow_plant(identifier, pos, node, stages)
...
function farming:add_gourd(full_unconnected_stem, connected_stem_basename, stem_itemstring, stem_def, stem_drop, gourd_itemstring, gourd_def, grow_interval, grow_chance, connected_stem_texture, gourd_on_construct_extra)
	...
	if stem_def.groups == nil then
		stem_def.groups = {dig_immediate=3, not_in_creative_inventory=1, plant=1, attached_node=1}
	end
	if stem_def.sounds == nil then
		stem_def.sounds = default.node_sound_leaves_defaults()
	end
		...
		action = function(stempos)
			...
			if light and light > 10 then
				...
				for n=#neighbors, 1, -1 do
					...
					if not ((minetest.get_item_group(floor.name, "soil") == 1 or floor.name=="default:dirt" or soilgroup == 2 or soilgroup == 3) and block.name == "air") then
				...
				if #neighbors > 0 then
					...
					if minetest.get_item_group(floor.name, "dirtifies_below_solid") == 1 then
						minetest.set_node(floorpos, {name = "default:dirt"})
					end
				end
			end
		end,
...
function farming:stem_color(startcolor, endcolor, step, step_count)
	local color = {}
